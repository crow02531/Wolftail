...
final class ComplementaryCheckVisitor implements DiffVisitor {
	...
	private DiffWriter dw;
	
	ComplementaryCheckVisitor(ByteBuf src) {
		dw = new DiffWriter();
		dw.setOutput(new CheckSameByteBuf(src));
	}
	...
	public void jzSetWeather(float rainStr, float thunderStr) {
		...
		dw.jzSetWeather(rainStr, thunderStr);
	}
	...
	public void jzSetTileEntity(ByteBuf buf) {
		...
		dw.jzSetTileEntity(buf);
	}
	
	private static class CheckSameByteBuf extends ByteBuf {
		
		ByteBuf buf;
		
		CheckSameByteBuf(ByteBuf expected) {
			buf = expected.slice();
		}
		...
		public ByteBuf writeByte(int value) {
			checkArgument(value == Byte.toUnsignedInt(buf.readByte()));
		...
		public ByteBuf writeShort(int value) {
			checkArgument(value == Short.toUnsignedInt(buf.readShort()));
		...
		public ByteBuf writeLong(long value) {
			checkArgument(value == buf.readLong());
		...
		public ByteBuf writeFloat(float value) {
			checkArgument(value == buf.readFloat());
		...
		public ByteBuf writeBytes(ByteBuf src) {
			checkArgument(ByteBufUtil.equals(src, 0, buf, buf.readerIndex(), src.readableBytes()));
